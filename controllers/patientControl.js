...
module.exports.showPatient = async (req, res) => {
  ...
  let upcomingAppointments; //Getting upcomming Appointments
  let pastAppointments; //Getting upcomming Appointments

  try {
    allAppointments = await AppointmentPatient.find({
      patient: patientId,
    })
      .populate({
        path: "appointment",
  ...
  //filter the upcomming appointments
  upcomingAppointments = allAppointments.filter((app) => {
    return app.appointment.date > currentDate;
  });

  //filter the pastAppointments
  pastAppointments = allAppointments.filter((app) => {
    return app.appointment.date < currentDate;
  });

  //Getting Prescription & Diagnostic Card
  try {
    DiagnosticCardAndPrescriptions =
      await DiagnosticCardAndPrescriptionController.getPrescription(patientId);
    DiagnosticCardAndPrescriptions =
      DiagnosticCardAndPrescriptions.slice().sort((a, b) => b.date - a.date);
  } catch (err) {
    console.log(err);
  }
  ...
  if (encodedDoctor) {
    ...
    res.render("patient/show", {
      ...
      pastAppointments,
    }); //In here parse the whole doctor object to front it can reduce it
  } else {
...
module.exports.getDoctorsAppointments = async (req, res) => {
  ...
  res.render("patient/showAppointments", {
    doctor,
    appointments,
    patient,
    paymentHash,
